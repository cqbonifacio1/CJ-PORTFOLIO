...
// Ported from CJ's provided component. Used on the Ending page's hand
// illustration band — dithers a source photo into a halftone pattern that
// reveals full color in a soft radius around the cursor.
//
// NOTE: `image` needs a real photo to dither. Until CJ supplies one at
// /public/images/ending/hands.jpg (or wherever), this falls back to the
// component's built-in placeholder photo so the section isn't broken —
// swap the `image` prop in Ending.tsx once the real asset is in place.
//
// ROTATION FIX: the Ending page renders this inside a CSS-rotated
// wrapper (rotate-90) so the band displays vertically full-screen. CSS
// rotation only affects what's painted on screen — it does NOT affect
// getBoundingClientRect() vs. clientWidth/clientHeight in the way you'd
// expect for mouse math: getBoundingClientRect() reports the ON-SCREEN
// (post-rotation) box, while the canvas itself is sized from
// clientWidth/clientHeight, which stay in the LOCAL (pre-rotation) frame.
// That mismatch made the reveal spot land nowhere near the real cursor
// once rotated. The `rotate` prop below tells this component which way
// it's been visually rotated so it can convert a real on-screen cursor
// position back into the correct local UV space before feeding it to the
// shader. Leave `rotate={0}` (the default) for any unrotated usage
// elsewhere — behavior there is unchanged from the original.
...
interface DitherRevealProps {
  ...
  waveDensity?: number;
  style?: CSSProperties;
  /** Match this to whatever CSS rotate(...) is applied to this
   *  component's container so mouse tracking stays correct. Only
   *  90-degree multiples are supported (matches Tailwind's rotate-90/
   *  180/270 classes). Default 0 = no correction, original behavior.
   *  If you use a NEGATIVE Tailwind rotation (e.g. -rotate-90, which
   *  is visually 270deg clockwise), pass rotate={270} — this prop
   *  tracks the effective clockwise angle, not the class name's sign. */
  rotate?: 0 | 90 | 180 | 270;
}
...
export default function DitherReveal(props: DitherRevealProps) {
  ...
  useEffect(() => {
    ...
    function onMove(e: PointerEvent) {
      ...
      // Raw fraction across the ON-SCREEN (post-rotation) box.
      const sx = (e.clientX - r.left) / r.width;
      const sy = (e.clientY - r.top) / r.height;

      // Map screen-space fraction back into the canvas's own local,
      // pre-rotation UV space, so the reveal lines up with the real
      // cursor no matter how the container is visually rotated via CSS.
      switch (rotate) {
        case 90:
          mouse.x = sy;
          mouse.y = sx;
